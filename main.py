...
with open(now+".sql", 'a') as sql_file:
    ...
    for i in range(1, length):
        for j in range(depth):
            sql_file.write(ins.insert("Game_Release_Date", "NULL, %d, %d, '%s', '%s'") % (random.randint(game_offset, i+game_offset), random.randint(platforms_offset, i+platforms_offset), pop.random_string(1000), pop.random_date()))
        sql_file.write(ins.insert("Game_Release_Date", "NULL, %d, %d, '%s', '%s'") % (i+game_offset, i+platforms_offset, pop.random_date(), pop.random_string(256)))
    sql_file.write("\n")
