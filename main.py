...
def tuple_true(tuple, index, offset):
    """
    updates the tuple to have a true value
    :param tuple: the tuple to modify
    :param index: the index to modify in the tuple
...
with open(now+".sql", 'a') as sql_file:
    ...
    for i in range(1, length):
        sql_file.write(ins.insert("Workers", "NULL, '%s', '%s', '%s', '%s', '%s'") % (pop.random_string(64), pop.random_string(64), pop.random_string(64), pop.random_string(1000), "https://fakeimg.pl/200x400/"))
    sql_file.write("\n")
    ...
    for i in range(1, length):
        i = i - 1
        for j in range(depth):
            setTrueIndex = random.randint(0, 4)

            format_list = tuple_true((random.randint(companies_offset, i+companies_offset), random.randint(workers_offset, i+workers_offset), pop.random_date(), pop.random_date(), False, False, False, False, False), setTrueIndex, 4)
            sql_file.write(ins.insert("Companies_Worker", "NULL, %d, %d, '%s', '%s', %r, %r, %r, %r, %r") % (format_list[0], format_list[1], format_list[2], format_list[3], format_list[4], format_list[5], format_list[6], format_list[7], format_list[8]))

        setTrueIndex = random.randint(0, 4)
        format_list = tuple_true((random.randint(companies_offset, i + companies_offset),
                                  random.randint(workers_offset, i + workers_offset), pop.random_date(),
                                  pop.random_date(), False, False, False, False, False), setTrueIndex, 4)
        sql_file.write(ins.insert("Companies_Worker", "NULL, %d, %d, '%s', '%s', %r, %r, %r, %r, %r") % (format_list[0], format_list[1], format_list[2], format_list[3], format_list[4], format_list[5], format_list[6], format_list[7], format_list[8]))
